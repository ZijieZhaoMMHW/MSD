...
for i=1:12
    index_here=time(:,2)==i;
    precip_clim(:,:,i)=naneman(precip(:,:,index_here),3);
end
    ...
    for j=1:size(precip_clim,2)
        ...
        if nansum(isnan(precip_here))~=length(precip_here)
            ...
            if isempty(p) || length(p)==1
                [~,n]=sort(precip_here);
                if ismember(n(end),[1 length(precip_here)]) && ismember(n(1),[1 length(precip_here)])
                    dur(i,j)=length(precip_here)-2;
                    range_msd=(2:(length(precip_here)-1))';
                    RD(i,j) = nansum(precip_here(range_msd).*num_day(period_here(range_msd)))./...
