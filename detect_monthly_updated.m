...
precip_cell=num2cell(precip_clim_sm,3);
...
if nansum(isnan(precip_here))~=length(precip_here)
    ...
    if ~isempty(p) && ~isempty(l) && length(p)>=2
        ...
        if (abs(l1-l2)+1)>=30 && (abs(l1-l2)+1)<=120
            depth_here=nanmean([p1 p2])-nanmean(precip_here(l_start:l_end));
            onset_here=l_start;
            ending_here=l_end;
        elseif (abs(l1-l2)+1)<=30 && length(p)>=3
            ...
            if (abs(l3-l1)+1)>=30 && (abs(l3-l1)+1)<=150 && (abs(l3-l2)+1)>=30 && (abs(l3-l2)+1)<=150
                ...
                depth_here=nanmean([p1 p2 p3])-nanmean(precip_here(l_start:l_end));
                onset_here=l_start;
                ending_here=l_end;
            else
                depth_here=nan;
