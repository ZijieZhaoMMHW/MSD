...
%
%  Reference
%   Angeles, M. E., Gonz¨¢lez, J. E., Ram¨ªrez?Beltr¨¢n, N. D., Tepley, C. A.,
...
BI=NaN(size(precip,1),size(precip,2),length(vmsd_start:vmsd_end));

for i=1:size(precip_clim,1)
    for j=1:size(precip_clim,2)
        precip_here=squeeze(precip(i,j,:));
        lat_here=lat_full(i,j);
        precip_clim_here=squeeze(precip_clim(i,j,:));
        if nansum(isnan(precip_here))~=length(precip_here)
            for y=vmsd_start:vmsd_end
                ...
                if length(precip_year)==12
                    ...
                    BI_here=BF.*((precip_a+precip_b)./(precip_clim_a+precip_clim_b));
                    BI(i,j,y-vmsd_start+1)=BI_here;
                end
            end
        end
    end
